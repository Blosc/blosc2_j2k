...
know codec id `39`.  The commands below rebuild only the HDF5 Blosc2 filter
...
export J2K_PACKAGE="$(python - <<'PY'
from pathlib import Path
import blosc2_j2k
print(Path(blosc2_j2k.__file__).resolve().parent)
PY
)"
...
# hdf5plugin only.  Backend choice comes from blosc2_j2k_plugins.json.
...
```bash
cd /tmp/blosc2_j2k_quickstart_XXXXXXXX
source .venv/bin/activate
export BLOSC2_PACKAGE="$(python - <<'PY'
from pathlib import Path
import blosc2
