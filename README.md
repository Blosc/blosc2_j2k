...
2. The c-blosc2 registry entry for `BLOSC_CODEC_J2K = 39` has been merged
   upstream.
3. python-blosc2 `4.4.3` exposes `blosc2.Codec.J2K` and ships a c-blosc2
   runtime that resolves codec id `39`.
4. The README demo quickstart and CI workflows now use the released
   `blosc2>=4.4.3` dependency instead of a temporary python-blosc2 branch.
5. Once hdf5plugin wheels are rebuilt against a c-blosc2 release with codec id
   `39`, the README demo can drop the local hdf5plugin source-build workaround.
...
9. Keep Grok as the open-source backend and Kakadu as an optional external
   backend.
