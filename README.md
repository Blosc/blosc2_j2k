...
There are two independent plugin layers when J2K data is read or written
through HDF5:
...
2. The Blosc2 layer must then resolve codec id `39`, whose registry name is
   `j2k`.
...
1. Try to load the codec library directly with the platform dynamic loader
   (`libblosc2_j2k.so`, `libblosc2_j2k.dylib`, or `blosc2_j2k.dll`).  This uses
   the usual runtime library search paths: RPATH, system library directories,
   `LD_LIBRARY_PATH` on Linux, `DYLD_LIBRARY_PATH` on macOS where permitted, or
   `PATH` on Windows.
2. Only if that direct lookup fails, and only if `python` or `python3` is
   ...
   ```bash
   python -c "import blosc2_j2k; blosc2_j2k.print_libpath()"
   ```
...
`hdf5plugin` is rebuilt against a c-blosc2 release containing codec id `39`,
...
This is the minimal in-process API path: register the J2K codec in the active
...
1. This repository now lives under the Blosc organization as the candidate
   `blosc2_j2k` plugin.
2. The c-blosc2 registry entry for `BLOSC_CODEC_J2K = 39` has been merged
   upstream.
3. The remaining upstream consolidation is the python-blosc2 update exposing
   `blosc2.Codec.J2K`.
4. Once python-blosc2 and hdf5plugin wheels are rebuilt against a c-blosc2
   release with codec id `39`, the README demo quickstart and CI workflows can
   drop the temporary source-build workarounds.
5. Keep validating Python, C/C++, HDF5, and service-runtime usage on Linux,
   macOS, and Windows wheels.
6. Keep the bootstrap only as a loader-order helper for HDF5-only deployments.
7. Keep the backend ABI independent from the Blosc2-facing codec.
8. Keep Grok as the open-source backend and Kakadu as an optional external
   backend.
