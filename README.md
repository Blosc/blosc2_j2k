...
```bash
git clone https://github.com/alemirone/blosc2_j2k.git
cd blosc2_j2k
...
```bash
python examples/quickstart.py --backend grok
```

and can exercise lossy rate mode:

```bash
python examples/quickstart.py --backend grok --lossy --codec-meta 80
```
...
For Python processes, importing `blosc2_j2k` registers the temporary codec id
