...
def hdf5_compression_opts(clevel=5, prefilter=HDF5_BLOSC2_NOFILTER):
    """Return low-level HDF5 Blosc2 filter options for the J2K codec id.
...
# On Windows, pre-load blosc2.dll before loading blosc2_j2k.dll
...
def get_libpath():
    ...
    if system == "Linux":
        candidates = ["libblosc2_j2k.so"]
    elif system == "Darwin":
        candidates = ["libblosc2_j2k.dylib", "libblosc2_j2k.so"]
    elif system == "Windows":
        candidates = [
            "blosc2_j2k.dll",
            "libblosc2_j2k.dll",
    ...
    if system == "Darwin":
        for pattern in ("libblosc2_j2k*.dylib", "libblosc2_j2k*.so"):
            for alt_path in pkg_dir.glob(pattern):
                return os.path.abspath(alt_path)
    if system == "Windows":
        for alt_path in pkg_dir.glob("blosc2_j2k*.pyd"):
            return os.path.abspath(alt_path)
...
def _add_windows_blosc2_dll_dirs():
    ...
    if not hasattr(os, "add_dll_directory"):
        return
    debug = os.environ.get("BLOSC2_J2K_DEBUG_DLL") == "1"
    ...
    if debug:
        print("blosc2_j2k: probing DLL dirs:", file=sys.stderr)
...
if os.name == "posix":
    ...
    # the same process.  Loading the codec globally makes `import blosc2_j2k`
...
lib.blosc2_j2k_configure.argtypes = [ctypes.POINTER(_RuntimeConfig)]
lib.blosc2_j2k_configure.restype = ctypes.c_int
lib.blosc2_j2k_list_plugins.argtypes = [ctypes.c_char_p, ctypes.c_size_t]
lib.blosc2_j2k_list_plugins.restype = ctypes.c_int
lib.blosc2_j2k_diagnose.argtypes = [ctypes.c_char_p, ctypes.c_size_t]
lib.blosc2_j2k_diagnose.restype = ctypes.c_int
lib.blosc2_j2k_last_error.argtypes = []
lib.blosc2_j2k_last_error.restype = ctypes.c_char_p
lib.blosc2_j2k_register_codec.argtypes = []
lib.blosc2_j2k_register_codec.restype = ctypes.c_int
...
def last_error():
    value = lib.blosc2_j2k_last_error()
    return value.decode("utf-8") if value else ""


def register_codec():
    """Verify that the active Blosc2 runtime knows the official J2K codec id."""
    rc = lib.blosc2_j2k_register_codec()
    if rc != 0:
        raise RuntimeError(last_error() or "failed to register blosc2_j2k codec")
...
def configure(plugin_path=None, backend=None, j2k_backend=None,
    ...
    directory installed next to ``libblosc2_j2k``.
    """
    if backend is not None and j2k_backend is not None and backend != j2k_backend:
        raise ValueError("backend and j2k_backend disagree")
    if backend is None:
        backend = j2k_backend
    ...
    rc = lib.blosc2_j2k_configure(ctypes.byref(cfg))
    if rc != 0:
        raise RuntimeError(last_error() or "blosc2_j2k_configure failed")


def _read_json_from_c(func):
    needed = func(None, 0)
    if needed < 0:
        raise RuntimeError(last_error() or "blosc2_j2k runtime query failed")
...
def list_plugins():
    return _read_json_from_c(lib.blosc2_j2k_list_plugins)


def diagnose():
    return _read_json_from_c(lib.blosc2_j2k_diagnose)
...
"""
    configure_call = ""
    if family == "j2k":
        configure_call = f"blosc2_j2k.configure(plugin_path={plugin_root!r}, backend={backend!r})"
...
def selftest(backends="available"):
    ...
    available = [
        p for p in plugins
        if p.get("family") == "j2k" and p.get("loadable") and p.get("abi_valid")
    ]
...
def _main(argv=None):
    ...
    parser = argparse.ArgumentParser(prog="python -m blosc2_j2k")
...
@atexit.register
def destroy():
    lib.blosc2_j2k_destroy()


params_defaults = {
    'tile_size': (0, 0),
    'tile_offset': (0, 0),
    # 'numlayers': 0, # blosc2_j2k C func set_params will still receive this param
    'quality_mode': None,
...
def set_params_defaults(**kwargs):
    ...
    >>> blosc2_j2k.set_default_params({'cod_format': blosc2_j2k.GrkFileFmt.GRK_FMT_J2K})
    and then call it again with some other parameters:
    >>> blosc2_j2k.set_default_params({'irreversible': True})
    the default for 'cod_format' will be restored to the original blosc2_j2k.GrkFileFmt.GRK_FMT_JP2 in grok.
    """
    # Check arguments
    ...
    lib.blosc2_j2k_set_default_params.argtypes = ([np.ctypeslib.ndpointer(dtype=np.int64)] * 2 +
    ...
    lib.blosc2_j2k_set_default_params(*args)
