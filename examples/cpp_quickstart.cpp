    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--codec-meta" && i + 1 < argc) {
            ...
            std::cout << "Usage: " << argv[0] << " [--codec-meta N] [--output-dir DIR]\n"
                      << "  Writes a raw HDF5 stack and a J2K-compressed HDF5 stack.\n"
...
Readback read_hdf5_stack_transparently(const std::string &filename) {
    ...
    for (hsize_t z = 0; z < static_cast<hsize_t>(kFrames); ++z) {
        ...
        if (filter_mask != 0) {
            fail("HDF5 chunk was written with the J2K filter marked as skipped");
        }
...
int main(int argc, char **argv) {
    ...
    blosc2_init();
    const int codec_id = blosc2_compname_to_compcode("j2k");
    if (codec_id != BLOSC_CODEC_J2K) {
        fail("updated c-blosc2 with registered codec name 'j2k' was not found");
    }

    std::filesystem::create_directories(options.output_dir);
    const std::string raw_file = options.output_dir + "/j2k_stack_raw_cpp.h5";
    const std::string compressed_file = options.output_dir + "/j2k_stack_blosc2_j2k_cpp.h5";
