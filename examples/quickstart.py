...
def assert_hdf5_filter_was_applied(filename, dataset_name):
    ...
    if skipped:
        lines = [
            "HDF5 wrote raw chunks because the Blosc2/J2K filter was skipped.",
            "The decoded values can still look correct, but this file is not J2K-compressed.",
            f"Make sure codec {blosc2_j2k.CODEC_ID} is known by the Blosc2 runtime used by the HDF5 filter.",
            "Skipped chunks:",
        ]
        lines.extend(
...
def main():
    parser = argparse.ArgumentParser(description="Hands-on blosc2_j2k roundtrip")
    parser.add_argument("--backend", default=None, help="backend name, for example grok or kakadu")
    ...
    parser.add_argument("--float-mode", choices=("uint8", "uint16", "uint32"), help="quantize float32 chunks before J2K compression")
    ...
    hdf5_demo = write_hdf5_stack_pair(data, Path(args.output_dir), blosc2_j2k.CODEC_ID)
    ...
    payload = {
        "codec": blosc2_j2k.CODEC_NAME,
        "codec_id": blosc2_j2k.CODEC_ID,
