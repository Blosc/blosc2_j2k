...
bool read_int32_array(MsgpackReader &reader,
                      ...
                      std::array<int32_t, BLOSC2_J2K_LAYOUT_MAX_DIM> &values) {
    uint32_t size = 0;
...
bool read_int64_array(MsgpackReader &reader,
                      int8_t expected_size,
                      std::array<int64_t, BLOSC2_J2K_LAYOUT_MAX_DIM> &values) {
    uint32_t size = 0;
    if (!reader.read_array_size(size) || size != static_cast<uint32_t>(expected_size)) {
        return false;
    }
...
bool parse_b2nd_meta(const uint8_t *content, int32_t content_len, B2ndLayout &layout) {
    ...
    if (!reader.read_integer(version) ||
        ...
        ndim <= 0 ||
        ndim > BLOSC2_J2K_LAYOUT_MAX_DIM) {
        return false;
    }
...
bool read_schunk_block_layout(blosc2_cparams *cparams, B2ndLayout &layout) {
    ...
    if (schunk->typesize <= 0 ||
        schunk->ndim <= 0 ||
        schunk->ndim > BLOSC2_J2K_LAYOUT_MAX_DIM ||
