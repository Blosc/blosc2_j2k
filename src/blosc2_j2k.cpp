...
using blosc2_j2k_detail::make_htj2k_decode_request;
...
using blosc2_j2k_detail::last_runtime_error;
...
// - blosc2_j2k_init(), blosc2_j2k_set_default_params(): public runtime setup.
...
void ensure_grok_initialized(uint32_t nthreads = 0, bool verbose = false) {
    ...
    if (!GRK_INITIALIZED) {
        blosc2_j2k_init(nthreads, verbose);
    }
}
...
grk_cparameters *current_compress_params(blosc2_cparams *cparams) {
    auto *codec_params = cparams ? (blosc2_j2k_params *)cparams->codec_params : nullptr;
...
void blosc2_j2k_init(uint32_t nthreads, bool verbose) {
    // initialize library
...
void blosc2_j2k_set_default_params(const int64_t *tile_size, const int64_t *tile_offset,
...
int blosc2_j2k_configure(const blosc2_j2k_runtime_config *config) {
    if (config == nullptr) {
...
int blosc2_j2k_register_codec(void) {
    ...
    if (existing >= 0) {
        set_runtime_error("Blosc2 codec name 'j2k' is already registered with another id");
        return -1;
    }
...
int blosc2_j2k_diagnose(char *buffer, size_t buffer_len) {
    return copy_string_to_c_buffer(diagnose_runtime_json(), buffer, buffer_len);
}

const char *blosc2_j2k_last_error(void) {
...
int blosc2_j2k_encoder(
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] blosc2_j2k_encoder called: meta=%d, input_len=%d\n", meta, input_len);
    }
    ...
    if (is_htj2k_requested(compress_params)) {
        fprintf(stderr,
                "[blosc2_j2k] HTJ2K encode parameters were passed to the J2K codec; "
                "use the separate 'htj2k' codec instead\n");
        return -1;
    }
...
static int blosc2_j2k_native_encoder_impl(
    ...
    auto *codec_params = (blosc2_j2k_params *)cparams->codec_params;
    ...
    if (is_htj2k_requested(compressParams)) {
        fprintf(stderr,
                "[blosc2_j2k] HTJ2K encode parameters were passed to the native "
                "J2K helper; use the separate 'htj2k' codec instead\n");
        if (codec_params == nullptr) {
            free(streamParams);
        }
        return -1;
...
int blosc2_j2k_decoder(const uint8_t *input, int32_t input_len, uint8_t *output, int32_t output_len,
    ...
    if (family == CodecFamily::UNKNOWN) {
        fprintf(stderr,
                "[blosc2_j2k] Could not identify JPEG2000 codestream family; "
                "refusing to guess a decoder backend\n");
        return -1;
    }
    if (family == CodecFamily::HTJ2K) {
        fprintf(stderr,
                "[blosc2_j2k] HTJ2K codestream was passed to the J2K codec; "
                "use the separate 'htj2k' codec instead\n");
        return -1;
    }
...
int blosc2_j2k_native_decoder(const uint8_t *input, int32_t input_len, uint8_t *output, int32_t output_len,
                               uint8_t meta, blosc2_dparams *dparams, const void *chunk) {
    (void)meta;
    (void)dparams;
    (void)chunk;
