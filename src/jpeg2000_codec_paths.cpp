/*********************************************************************
 * blosc2_j2k: J2K and HTJ2K codec dispatch paths.
 *
...
extern "C" int blosc2_j2k_native_encoder(const uint8_t *input,
                                           int32_t input_len,
                                           uint8_t *output,
                                           int32_t output_len,
                                           uint8_t meta,
...
extern "C" int blosc2_j2k_native_decoder(const uint8_t *input,
                                           int32_t input_len,
                                           uint8_t *output,
                                           int32_t output_len,
                                           uint8_t meta,
...
int encode_j2k_with_plugin_or_native(const uint8_t *input,
    ...
    if (plugin) {
        if (!plugin->vtable.supports(&request)) {
            fprintf(stderr,
                    "[blosc2_j2k] J2K plugin %s does not support requested layout "
                    "(precision=%u components=%u)\n",
                    plugin->name, request.precision_bits, request.num_components);
            return -1;
        }
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Using J2K plugin: %s %s\n", plugin->name, plugin->version);
        }
...
int encode_htj2k_with_plugin(const uint8_t *input,
    ...
    if (!plugin) {
        fprintf(stderr,
                "[blosc2_j2k] HTJ2K encoding requires an HTJ2K backend; configure "
                "the separate blosc2_htj2k package or BLOSC2_HTJ2K_BACKEND\n");
        return -1;
    }
    if (!plugin->vtable.supports(&request)) {
        fprintf(stderr,
                "[blosc2_j2k] HTJ2K plugin %s does not support requested layout "
                "(precision=%u components=%u)\n",
                plugin->name, request.precision_bits, request.num_components);
        return -1;
    }
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Using HTJ2K plugin: %s %s\n", plugin->name, plugin->version);
    }
    return plugin->vtable.encode(input, input_len, output, output_len, meta, cparams, chunk, &request);
}
...
int decode_j2k_with_plugin_or_native(const uint8_t *input,
    ...
    if (plugin) {
        if (!plugin->vtable.supports(&request)) {
            fprintf(stderr, "[blosc2_j2k] J2K plugin %s does not support this decode request\n",
                    plugin->name);
            return -1;
        }
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Using J2K plugin decoder: %s %s\n",
                    plugin->name, plugin->version);
        }
        return plugin->vtable.decode(input, input_len, output, output_len, meta, dparams, chunk, &request);
    }

    return blosc2_j2k_native_decoder(input, input_len, output, output_len, meta, dparams, chunk);
}
...
int decode_htj2k_with_plugin(const uint8_t *input,
    ...
    if (!plugin) {
        fprintf(stderr,
                "[blosc2_j2k] HTJ2K decoding requires an HTJ2K backend; configure "
                "the separate blosc2_htj2k package or BLOSC2_HTJ2K_BACKEND\n");
        return -1;
    }
    if (!plugin->vtable.supports(&request)) {
        fprintf(stderr, "[blosc2_j2k] HTJ2K plugin %s does not support this decode request\n",
                plugin->name);
        return -1;
    }
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Using HTJ2K plugin decoder: %s %s\n",
                plugin->name, plugin->version);
    }
    return plugin->vtable.decode(input, input_len, output, output_len, meta, dparams, chunk, &request);
}
