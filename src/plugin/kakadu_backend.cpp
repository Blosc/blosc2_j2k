...
int kakadu_encode(
    ...
    if (!(typesize == 1 || typesize == 2)) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu encode error: unsupported typesize=%d (only 1 and 2 are supported)\n",
                    typesize);
        }
        return -1;
    }
...
int kakadu_decode(
    ...
    try {
        ...
        if (!(precision == 8 || precision == 16)) {
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu: unsupported precision=%d (only 8 and 16 supported)\n", precision);
            }
            codestream.destroy();
            return -1;
        }
