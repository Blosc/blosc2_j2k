/*********************************************************************
 * blosc2_j2k: Kakadu (JPEG2000 codec) backend for Blosc2
 *
...
class KduErrorHandler : public kdu_message {
  public:
    ...
    void flush(bool end_of_message) override {
        if (end_of_message) {
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu error: %s\n", buffer.c_str());
            }
            buffer.clear();
...
class KduWarningHandler : public kdu_message {
  public:
    ...
    void flush(bool end_of_message) override {
        if (end_of_message) {
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu warning: %s\n", buffer.c_str());
            }
            buffer.clear();
        }
    }
...
bool kakadu_extra_has_param(const char *param) {
    const char *extra = std::getenv("BLOSC2_J2K_KAKADU_PARAMS");
...
void apply_kakadu_mode(siz_params &siz, bool htj2k) {
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu mode: %s\n", htj2k ? "HTJ2K" : "J2K");
    }
}
...
void apply_kakadu_overrides(siz_params &siz) {
    ...
    if (extra != nullptr && *extra != '\0') {
        ...
        while (start < s.size()) {
            ...
            if (!tok.empty()) {
                ...
                if (debug) {
                    fprintf(stderr, "[blosc2_j2k] Kakadu param: %s (%s)\n",
    ...
    const char *clevels_s = std::getenv("BLOSC2_J2K_CLEVELS");
    if (clevels_s != nullptr && *clevels_s != '\0') {
        ...
        if (end != clevels_s && end && *end == '\0') {
            ...
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu Clevels=%ld (%s)\n",
                        clevels, ok ? "ok" : "FAILED");
            }
        } else if (debug) {
            fprintf(stderr, "[blosc2_j2k] Ignoring invalid BLOSC2_J2K_CLEVELS=%s\n", clevels_s);
        }
    }
}

// Choose conservative defaults for lossless/lossy operation unless explicitly
// overridden by BLOSC2_J2K_KAKADU_PARAMS.
void apply_kakadu_rate_defaults(siz_params &siz, int32_t precision, bool rate_controlled) {
    ...
    if (!rate_controlled) {
        if (!kakadu_extra_has_param("creversible")) {
            ...
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu lossless default: Creversible=yes (%s)\n",
                        ok ? "ok" : "FAILED");
            }
        }
        return;
    ...
    if (!kakadu_extra_has_param("creversible")) {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu rate default: Creversible=no (%s)\n",
                    ok ? "ok" : "FAILED");
        }
    }
    if (!kakadu_extra_has_param("qstep") && !kakadu_extra_has_param("qfactor")) {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu rate default: %s (%s)\n",
                    cmd, ok ? "ok" : "FAILED");
        }
    }
}
...
void apply_kakadu_precision_defaults(siz_params &siz, int32_t precision) {
    ...
    if (precision == 32 &&
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu uint32 default: Clevels=3 (%s)\n",
                    ok ? "ok" : "FAILED");
        }
    }
}
...
KakaduTune get_kakadu_tune() {
    KakaduTune t;
    t.force_precise = env_flag("BLOSC2_J2K_KAKADU_PRECISE", true);
    t.want_fastest = env_flag("BLOSC2_J2K_KAKADU_FAST", false);
    t.threads = env_int("BLOSC2_J2K_KAKADU_THREADS", 0);
    return t;
}
...
int kakadu_encode(
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu tune: force_precise=%d want_fastest=%d threads=%d\n",
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu encoder path selected (%s, %s)\n",
                htj2k ? "HTJ2K" : "J2K",
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu has_b2nd=%d dim_x=%ld dim_y=%ld num_comps=%d typesize=%d\n",
    ...
    if (!has_b2nd) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu encoder error: no b2nd metadata found\n");
        }
        return -1;
    }
    ...
    if (!(typesize == 1 || typesize == 2 || typesize == 4)) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu encode error: unsupported typesize=%d (only 1, 2 and 4 are supported)\n",
                    typesize);
        }
        return -1;
    }
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu encode dims=%ldx%ld comps=%d typesize=%d input_len=%d expected=%ld output_len=%d\n",
                static_cast<long>(dim_x), static_cast<long>(dim_y), num_comps, typesize,
                input_len, static_cast<long>(expected_len), output_len);
    }
    if (input_len < expected_len) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu encode error: input_len too small\n");
        }
        return -1;
    }
    siz_params siz;
    ...
    if (signed_32bit_rate_control) {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu uint32 rate-control path: using signed 32-bit samples\n");
        }
    }
    ...
    try {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu push_stripe needs_more=%d\n", needs_more ? 1 : 0);
        }
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu encoder exception\n");
        }
        return -1;
    }

    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu encoded bytes=%zu\n", target.data.size());
    }
    if (static_cast<int32_t>(target.data.size()) > output_len) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu encode error: output buffer too small\n");
        }
        return 0;
    }
...
int kakadu_decode(
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu decoder path selected (%s)\n",
    ...
    try {
        if (is_jp2) {
            ...
            if (hdr <= 0) {
                if (debug) {
                    fprintf(stderr, "[blosc2_j2k] Kakadu JP2 header not ready or incompatible\n");
                }
                return -1;
            }
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: num_comps=%d\n", num_comps);
        }
        if (!((num_comps == 1) || (num_comps == 3))) {
            codestream.destroy();
            return -1;
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: dims.x=%ld, dims.y=%ld\n", (long)dims.size.x, (long)dims.size.y);
        }
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: codestream precision=%d bits signed=%d\n",
        ...
        if (!(precision == 8 || precision == 16 || precision == 32)) {
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu: unsupported precision=%d (only 8, 16 and 32 supported)\n", precision);
            }
            codestream.destroy();
            return -1;
        }
        ...
        thread_env.setup(tune.threads);
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: starting stripe decompressor\n");
        }
        ...
        std::vector<int> precisions(num_comps, precision);
        const int requested_stripe_height = env_int("BLOSC2_J2K_KAKADU_DECODE_STRIPE_HEIGHT", 128);
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: pulling stripes height<=%d\n",
                    max_stripe_height);
        }
        ...
        while (rows_done < height) {
            ...
            rows_done += rows;
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu decoder: pulled rows=%d/%d needs_more=%d\n",
        ...
        if (rows_done != height) {
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Kakadu decoder error: incomplete stripe decode rows=%d/%d\n",
                        rows_done, height);
            }
            decompressor.reset();
            codestream.destroy();
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: resetting stripe decompressor\n");
        }
        decompressor.reset();
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: stripe decompressor reset done\n");
        }
        if (typesize == 1) {
        ...
        codestream.destroy();
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Kakadu decoder: codestream destroyed\n");
        }
        if (is_jp2) {
            jp2.close();
            family.close();
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] Kakadu decoder: complete\n");
    }
    return output_len;
}
