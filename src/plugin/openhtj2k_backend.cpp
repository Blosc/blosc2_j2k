/*********************************************************************
 * blosc2_j2k: OpenHTJ2K backend for HTJ2K replacement compression.
 *
...
extern "C" int blosc2_openhtj2k_encoder(
    ...
    if (!blosc2_openhtj2k_supports(request)) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] OpenHTJ2K encode does not support this HTJ2K request\n");
        }
        return -1;
    }
    ...
    // blosc2_j2k uses codec_meta / 10.0 as a target compression ratio.  The
    ...
    try {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] OpenHTJ2K encoded bytes=%zu\n", total_size);
        }
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] OpenHTJ2K encoder exception: %s\n", exc.what());
        }
        return -1;
    }
}
...
extern "C" int blosc2_openhtj2k_decoder(
    ...
    try {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] OpenHTJ2K decoder exception: %s\n", exc.what());
        }
        return -1;
    }
}
