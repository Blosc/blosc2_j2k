/*********************************************************************
 * blosc2_j2k: runtime replacement plugin discovery and loading.
 *
...
plugin_library_handle_t open_plugin_library(const fs::path &libpath,
    ...
    if (!handle) {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] LoadLibrary failed for %s: %s\n",
    ...
    if (!handle) {
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] dlopen failed for %s: %s\n",
...
j2k_codec_plugin_t* find_j2k_plugin_descriptor(plugin_library_handle_t handle,
    ...
    if (!symbol) {
        if (debug) {
            std::string path = libpath.string();
            fprintf(stderr, "[blosc2_j2k] GetProcAddress failed for %s: %s\n",
    ...
    if (dlsym_error) {
        if (debug) {
            std::string path = libpath.string();
            fprintf(stderr, "[blosc2_j2k] dlsym failed for %s: %s\n", path.c_str(), dlsym_error);
        }
        return nullptr;
    }
    return reinterpret_cast<j2k_codec_plugin_t*>(symbol);
...
bool is_valid_j2k_plugin_descriptor(const j2k_codec_plugin_t *plugin,
    ...
    if (!plugin) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Missing J2K plugin descriptor in %s\n", path.c_str());
        }
        return false;
    }
    if (plugin->abi_version != J2K_CODEC_PLUGIN_ABI_VERSION) {
        if (debug) {
            fprintf(stderr,
                    "[blosc2_j2k] J2K plugin ABI mismatch in %s: got %u, expected %u\n",
    ...
    if (plugin->struct_size < sizeof(j2k_codec_plugin_t)) {
        if (debug) {
            fprintf(stderr,
                    "[blosc2_j2k] Plugin descriptor too small in %s: got %u, expected at least %zu\n",
    ...
    if (!plugin->name || !plugin->version || !plugin->vtable.supports ||
        ...
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Incomplete J2K plugin descriptor in %s\n", path.c_str());
        }
        return false;
    }
    return true;
...
bool is_valid_htj2k_plugin_descriptor(const htj2k_codec_plugin_t *plugin,
    ...
    if (!plugin) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Missing HTJ2K plugin descriptor in %s\n", path.c_str());
        }
        return false;
    }
    if (plugin->abi_version != HTJ2K_CODEC_PLUGIN_ABI_VERSION) {
        if (debug) {
            fprintf(stderr,
                    "[blosc2_j2k] HTJ2K plugin ABI mismatch in %s: got %u, expected %u\n",
    ...
    if (plugin->struct_size < sizeof(htj2k_codec_plugin_t)) {
        if (debug) {
            fprintf(stderr,
                    "[blosc2_j2k] HTJ2K plugin descriptor too small in %s: got %u, expected at least %zu\n",
    ...
    if (!plugin->name || !plugin->version || !plugin->vtable.supports ||
        !plugin->vtable.encode || !plugin->vtable.decode) {
        if (debug) {
            fprintf(stderr, "[blosc2_j2k] Incomplete HTJ2K plugin descriptor in %s\n", path.c_str());
        }
        return false;
    }
    return true;
...
j2k_codec_plugin_t* load_j2k_replacement_plugin() {
    ...
    bool debug = (getenv("BLOSC2_J2K_DEBUG") != nullptr);
    for (const PluginCandidate &candidate : candidates) {
        if (candidate.native) {
            if (debug) {
                fprintf(stderr, "[blosc2_j2k] Using native J2K backend\n");
            }
            return nullptr;
        }
        for (const fs::path &libpath : candidate_library_paths(candidate)) {
            ...
            if (is_valid_j2k_plugin_descriptor(plugin, libpath, debug)) {
                ...
                if (debug) {
                    std::string path = libpath.string();
                    fprintf(stderr, "[blosc2_j2k] Loaded J2K plugin: %s %s from %s\n",
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] No valid J2K plugin found in configured candidates\n");
    }
    return nullptr;
}
...
htj2k_codec_plugin_t* load_htj2k_replacement_plugin() {
    ...
    for (const PluginCandidate &candidate : candidates) {
        for (const fs::path &libpath : candidate_library_paths(candidate)) {
            ...
            if (is_valid_htj2k_plugin_descriptor(plugin, libpath, debug)) {
                ...
                if (debug) {
                    std::string path = libpath.string();
                    fprintf(stderr, "[blosc2_j2k] Loaded HTJ2K plugin: %s %s from %s\n",
                            plugin->name, plugin->version, path.c_str());
                }
                return plugin;
            }
    ...
    if (debug) {
        fprintf(stderr, "[blosc2_j2k] No valid HTJ2K plugin found in configured candidates\n");
    }
    return nullptr;
}
