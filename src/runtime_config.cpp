/*********************************************************************
 * blosc2_j2k: runtime configuration and plugin candidate discovery.
 *
...
FloatRuntimeConfig resolve_float_config_locked(const RuntimeConfig &config) {
    ...
    std::string mode = env_string("BLOSC2_J2K_FLOAT");
    if (mode.empty()) {
        return result;
    }
    if (!parse_float_quant_bits(mode, result.enabled, result.quant_bits)) {
        result.valid = false;
        result.error = "invalid BLOSC2_J2K_FLOAT value: " + mode;
        return result;
    }

    std::string clamp_min = env_string("BLOSC2_J2K_FLOAT_CLAMP_MIN");
    if (!clamp_min.empty()) {
        result.clamp_min_set = true;
        if (!parse_double_value(clamp_min, result.clamp_min)) {
            result.valid = false;
            result.error = "invalid BLOSC2_J2K_FLOAT_CLAMP_MIN value: " + clamp_min;
            return result;
        }
    }
    std::string clamp_max = env_string("BLOSC2_J2K_FLOAT_CLAMP_MAX");
    if (!clamp_max.empty()) {
        result.clamp_max_set = true;
        if (!parse_double_value(clamp_max, result.clamp_max)) {
            result.valid = false;
            result.error = "invalid BLOSC2_J2K_FLOAT_CLAMP_MAX value: " + clamp_max;
            return result;
        }
    }
    std::string nan_policy = env_string("BLOSC2_J2K_FLOAT_NAN_POLICY");
    if (!nan_policy.empty() && nan_policy != "fail") {
        result.valid = false;
        result.error = "blosc2_j2k float mode v1 only supports BLOSC2_J2K_FLOAT_NAN_POLICY=fail";
        return result;
    }
    result.nan_policy = 0;
    return validate_float_config(result);
...
std::string legacy_env_name(PluginFamily family) {
    return family == PluginFamily::J2K ? "BLOSC2_J2K_REPLACEMENT_DIR"
                                       : "";
}

std::string named_backend_env_name(PluginFamily family) {
    return family == PluginFamily::J2K ? "BLOSC2_J2K_BACKEND"
                                       : "";
}
...
std::string configured_plugin_path_without_manifest_locked(const RuntimeConfig &config) {
    ...
    return env_string("BLOSC2_J2K_PLUGIN_PATH");
}
...
fs::path manifest_path() {
    return self_library_dir() / "blosc2_j2k_plugins.json";
}
...
int configure_runtime(const char *plugin_path,
    ...
    if (config.frozen) {
        config.last_error = "blosc2_j2k runtime is already in use; configure before first encode/decode";
        return -1;
    }
    ...
    if ((float_flags & ~0x07u) != 0) {
        config.last_error = "unknown blosc2_j2k float configuration flag";
        return -1;
    }
...
std::vector<PluginCandidate> plugin_inventory_candidates() {
    ...
    for (PluginFamily family : {PluginFamily::J2K}) {
        std::string legacy_dir = env_string(legacy_env_name(family).c_str());
        if (!legacy_dir.empty()) {
...
std::string runtime_diagnostics_json() {
    ...
    append_json_string_field(out, "backend", configured_backend_locked(PluginFamily::J2K, config));
    append_json_string_field(out, "legacy_j2k_dir", env_string("BLOSC2_J2K_REPLACEMENT_DIR"));
    ...
    out << "]";
    out << ",\"env\":{";
    append_env_json_field(out, "BLOSC2_J2K_PLUGIN_PATH");
    append_env_json_field(out, "BLOSC2_J2K_BACKEND");
    append_env_json_field(out, "BLOSC2_J2K_REPLACEMENT_DIR");
    append_env_json_field(out, "BLOSC2_J2K_FLOAT");
    append_env_json_field(out, "BLOSC2_J2K_FLOAT_CLAMP_MIN");
    append_env_json_field(out, "BLOSC2_J2K_FLOAT_CLAMP_MAX");
    append_env_json_field(out, "BLOSC2_J2K_FLOAT_NAN_POLICY");
    append_env_json_field(out, "BLOSC2_J2K_LIBRARY");
    append_env_json_field(out, "BLOSC2_J2K_DEBUG");
